...
// TODO : guard page, 어떻게 페이지를 보호하는가, 페이지 폴트, 코드구현
	...
	주로 사용자 공간 스택에서 사용된다.
	관리 주체 : 커널의 메모리 관리(MM) 서브시스템, 가상 메모리 매핑(VMA)과 페이지 테이블 설정을 통해 구현
	페이지는 어떻게 보호되나?
	페이지 보호는 CPU의 MMU가 수행하고, 커널은 페이지 테이블(PTE)에 권한 비트를 설정한다.
	보호 방식 : 페이지를 아예 매핑하지 않거나(Present=0), 읽기/쓰기/유저 접근 권한을 제거
	역할 분담 :	보호 실행 -> MMU(하드웨어),	보호 설정 : 커널 MM 서브시스템(소프트웨어)
	페이지 폴트가 발생하면?
	페이지 접근이 실패하면 CPU가 페이지 폴트 예외를 발생시킨다.
	처리 주체 : CPU -> 폴트 발생, 커널 -> 페이지 폴트 핸들러가 처리
	처리 결과 : 사용자 모드에서 발생한 페이지 폴트는 유효한 VMA에 대한 접근으로 커널이 처리 가능한 경우(recoverable page fault)와,
	주소 공간 정의 또는 접근 권한을 위반해 복구할 수 없는 경우(unrecoverable page fault)로 나뉘며,
	전자는 커널이 페이지를 할당·매핑해 실행을 계속하고, 후자는 SIGSEGV 또는 SIGBUS로 프로세스를 종료한다.
	커널 모드 -> 복구 불가능한 경우가 많아 치명적(패닉)일 수 있음
...
// TODO : 해시테이블 단점
	...
	메모리 오버헤드
	버킷 배열 + 버킷별 락 + 헤드 등이 필요해서 단순 리스트보다 구조가 무겁다. (대신 성능을 얻는 trade-off)
	순서/정렬 정보가 없다
	범위 검색, 정렬된 순회 같은 건 부적합

	해시 테이블(obj_hash[])
	같은 해시값 충돌
	같은 해시값으로 충돌하면, 그 버킷 안의 hlist를 순회해서 주소를 직접 비교
	debug_objects에서 해시 충돌이 나면, 같은 버킷의 hlist를 순회하면서 debug_obj->object(필요시 descr)를 비교해 엔트리를 찾고,
	있으면 상태 머신 검사를 진행하고, 없으면 새 debug_obj를 풀에서 할당해 버킷 리스트에 추가
...
// TODO : hlist 목적, 왜?, 장단점, 버킷
	...
	삭제가 O(1) 이다
	hlist_node가 pprev(이중 포인터)를 들고 있어서, 이 노드를 가리키던 포인터를 바로 고칠 수 있다.
	→ 중간 노드 삭제도 빠름.
	장점
	메모리 오버헤드가 작다
	일반 list_head(next+prev)보다 가벼운 편(특히 헤드가 단순).
	해시 버킷에서 흔히 쓰는 패턴(추가/삭제/순회)에 적합
	중간 삭제 O(1) (pprev 덕분)
	단점
	양방향 순회가 불편
	prev가 아니라 pprev라서 이전 노드로 이동 같은 건 일반 list보다 덜 직관적.
	정렬/인덱스 접근 같은 건 불가
	결국 리스트이므로 순회는 필요.
	버킷 : 해시테이블을 여러 칸으로 나눈 각각의 칸
	충돌이 나면 같은 버킷에 여러 노드가 달리고, 그 안에서 주소 직접 비교로 정확한 debug_obj를 찾는다.
	버킷은 해시 결과로 선택되는 슬롯이고, 그 슬롯 안에 락과 hlist가 들어 있어 충돌 엔트리들을 묶어 관리
